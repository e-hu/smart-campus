...
class FileService {
    ...
    public static function getFileUrl($filename, $storage = null) {
        ...
        switch (empty($storage) ? sysconf_v('storage_type') : $storage) {
            case 'local':
    ...
    public static function getUploadQiniuUrl($isClient = true) {
        $region = sysconf_v('storage_qiniu_region');
        $isHttps = !!sysconf_v('storage_qiniu_is_https');
        switch ($region) {
            case '华东':
                if ($isHttps) {
    ...
    public static function getUploadOssUrl() {
        return (request()->isSsl() ? 'https' : 'http') . '://' . sysconf_v('storage_oss_domain');
    }
    ...
    public static function getBaseUriQiniu() {
        return (sysconf_v('storage_qiniu_is_https') ? 'https' : 'http') . '://' . sysconf_v('storage_qiniu_domain') . '/';
    }
    ...
    public static function getBaseUriOss() {
        return (sysconf_v('storage_oss_is_https') ? 'https' : 'http') . '://' . sysconf_v('storage_oss_domain') . '/';
    }
    ...
    public static function hasFile($filename, $storage = null) {
        switch (empty($storage) ? sysconf_v('storage_type') : $storage) {
            ...
            case 'qiniu':
                $auth = new Auth(sysconf_v('storage_qiniu_access_key'), sysconf_v('storage_qiniu_secret_key'));
                $bucketMgr = new BucketManager($auth);
                list($ret, $err) = $bucketMgr->stat(sysconf_v('storage_qiniu_bucket'), $filename);
                return $err === null;
            case 'oss':
                $ossClient = new OssClient(sysconf_v('storage_oss_keyid'), sysconf_v('storage_oss_secret'), self::getBaseUriOss(), true);
                return $ossClient->doesObjectExist(sysconf_v('storage_oss_bucket'), $filename);
        }
        return false;
    }
    ...
    public static function readFile($filename, $storage = null) {
        switch (empty($storage) ? sysconf_v('storage_type') : $storage) {
            ...
            case 'oss':
                $ossClient = new OssClient(sysconf_v('storage_oss_keyid'), sysconf_v('storage_oss_secret'), self::getBaseUriOss(), true);
                return $ossClient->getObject(sysconf_v('storage_oss_bucket'), $filename);
        }
    ...
    public static function save($filename, $content, $file_storage = null) {
        $type = empty($file_storage) ? sysconf_v('storage_type') : $file_storage;
    ...
    public static function qiniu($filename, $content) {
        $auth = new Auth(sysconf_v('storage_qiniu_access_key'), sysconf_v('storage_qiniu_secret_key'));
        $token = $auth->uploadToken(sysconf_v('storage_qiniu_bucket'));
    ...
    public static function oss($filename, $content) {
        try {
            $ossClient = new OssClient(sysconf_v('storage_oss_keyid'), sysconf_v('storage_oss_secret'), self::getBaseUriOss(), true);
            $result = $ossClient->putObject(sysconf_v('storage_oss_bucket'), $filename, $content);
