...
class Tags extends BasicAdmin {
    ...
    public function index() {
        ...
        $db = Db::name($this->table)->where('appid', $wechat->appid)->order('id asc');
        foreach (['name'] as $key) {
    ...
    public function add() {
        ...
        if (WechatService::syncFansTags()) {
            LogService::write('微信管理', '添加完成后同步全部微信粉丝标签成功');
            $this->success('添加完成后同步获取粉丝标签成功!', '');
        }
    ...
    public function edit() {
        ...
        if (WechatService::syncFansTags()) {
            LogService::write('微信管理', '编辑完成后同步全部微信粉丝标签成功');
            $this->success('编辑完成后同步获取粉丝标签成功!', '');
        }
    ...
    public function del() {
        // 显示编辑界面
        $wechat = &load_wechat("User");
        Db::name($this->table)->where('appid', $wechat->appid)->where('id',$_POST['id'])->delete();
        if (false === ($result = $wechat->delTag($_POST['id']))) {
            $this->error("添加粉丝标签失败. {$wechat->errMsg}[{$wechat->errCode}]");
        }
        if (WechatService::syncFansTags()) {
            LogService::write('微信管理', '删除完成后同步全部微信粉丝标签成功');
            $this->success('删除完成后同步获取粉丝标签成功!', '');
        }
