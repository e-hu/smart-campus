...
class Fans extends BasicAdmin {
    ...
    public function index() {
        ...
        $db = Db::name($this->table)->where('appid', $wechat->appid)->where('is_back', '0')->order('subscribe_time desc');
        ...
        if (isset($get['tag']) && $get['tag'] !== '') {
            ...
            $db->where("',' +''+tagid_list+''+',' like :tag", ['tag' => "%,{$get['tag']},%"]);
        }
    ...
    protected function _data_filter(&$list) {
        $wechat = &load_wechat("User");
        $tags = Db::name('WechatFansTags')->where('appid', $wechat->appid)->column('id,name');
        foreach ($list as &$vo) {
    ...
    public function back() {
        ...
        $wechat = &load_wechat("User");
        $db = Db::name($this->table)->where('appid', $wechat->appid)->where('is_back', '1')->order('subscribe_time desc');
        $get = $this->request->get();
        !empty($get['sex']) && $db->where('sex', $get['sex']);
        foreach (['nickname', 'country', 'province', 'city'] as $key) {
            if (isset($get[$key]) && $get[$key] !== '') {
    ...
    public function tagset() {
        ...
        $wechat = & load_wechat('User');
        $fans = Db::name('WechatFans')->where('appid', $wechat->appid)->where('id', $fans_id)->find();
        ...
        if (false !== Db::name('WechatFans')->where('appid', $wechat->appid)->where('id', $fans_id)->setField('tagid_list', $tags)) {
            $this->success('粉丝标签成功!', '');
        }
    ...
    private function _getActionOpenids() {
        ...
        $wechat = &load_wechat("User");
        $openids = Db::name($this->table)->where('appid', $wechat->appid)->where('id', 'in', explode(',', $ids))->column('openid');
    ...
    public function sync() {
        ...
        $this->error('同步获取粉丝失败，请稍候再试!,请先将本机的IP地址填入开发者模式的白名单!');
    }
