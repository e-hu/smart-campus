...
class News extends BasicAdmin {
    ...
    public function index() {
        ...
        $db = Db::name($this->table)->where('appid',$wechat->appid)->where('is_deleted', '0')->order('id desc');
    ...
    public function image() {
        ...
        $wechat = &load_wechat("User");
        return $this->_list(Db::name('WechatNewsMedia')->where('appid',$wechat->appid)->where('type', 'image'));
    }
    ...
    public function add() {
        ...
        if ($this->request->isPost()) {
            ...
            if (($ids = $this->_apply_news_article($data['data'])) && !empty($ids)) {
                $wechat = &load_wechat("User");
                $post = ['article_id' => $ids, 'create_by' => session('user.id'),'appid'=>$wechat->appid];
    ...
    public function edit() {
        ...
        if (!empty($ids)) {
            $wechat = &load_wechat("User");
            $post = ['id' => $id, 'article_id' => $ids, 'create_by' => session('user.id'),'appid'=>$wechat->appid];
    ...
    protected function _apply_news_article($data, $ids = []) {
        foreach ($data as &$vo) {
            ...
            if (empty($vo['id'])) {
                ...
                $id = intval($vo['id']);
                unset($vo['id']);
                $result = Db::name('WechatNewsArticle')->where('appid',$wechat->appid)->where('id', $id)->update($vo);
            }
            if ($result !== FALSE) {
                $ids[] = $id;
            }
    ...
    public function del() {
        ...
        if (DataService::update($this->table,$where)) {
            $this->success("图文删除成功!", '');
        }
