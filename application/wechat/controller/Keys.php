...
class Keys extends BasicAdmin {
    ...
    public function index() {
        ...
        $db = Db::name($this->table)->where('appid',$wechat->appid)->where('keys', 'not in', ['subscribe', 'default']);
    ...
    protected function _form_filter($data) {
        if ($this->request->isPost() && isset($data['keys'])) {
            ...
            $db = Db::name($this->table)->where('appid',$wechat->appid)->where('keys', $data['keys']);
    ...
    protected function _subscribe_form_filter(&$data) {
        ...
        if ($this->request->isGet()) {
            $data = Db::name($this->table)->where('appid',$wechat->appid)->where('keys', 'subscribe')->find();
        }
        $data['keys'] = 'subscribe';
        $data['appid'] = $wechat->appid;
    ...
    protected function _defaults_form_filter(&$data) {
        $wechat = &load_wechat("User");
        if ($this->request->isGet()) {
            $data = Db::name($this->table)->where('appid',$wechat->appid)->where('keys', 'default')->find();
        }
        $data['keys'] = 'default';
        $wechat = &load_wechat("User");
        $data['appid'] = $wechat->appid;
