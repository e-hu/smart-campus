...
class Plugs extends BasicAdmin {
    ...
    public function upstate() {
        ...
        switch (strtolower($post['uptype'])) {
            ...
            case 'oss':
                ...
                $config['signature'] = base64_encode(hash_hmac('sha1', $config['policy'], sysconf_v('storage_oss_secret'), true));
                $config['OSSAccessKeyId'] = sysconf_v('storage_oss_keyid');
        }
    ...
    protected function _getQiniuToken($key) {
        $accessKey = sysconf_v('storage_qiniu_access_key');
        $secretKey = sysconf_v('storage_qiniu_secret_key');
        $bucket = sysconf_v('storage_qiniu_bucket');
        $host = sysconf_v('storage_qiniu_domain');
        $protocol = sysconf_v('storage_qiniu_is_https') ? 'https' : 'http';
        $params = [
            "scope"      => "{$bucket}:{$key}",
            "deadline"   => 3600 + time(),
