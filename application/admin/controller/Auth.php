    ...
    public function apply() {
        ...
        switch (strtolower($this->request->get('action', '0'))) {
            case 'getnode':
                ...
                if(session('user.company_id') != '0'){         //公司帐号
                    $auth_ids = session('user.authorize'); //账户拥有的权限
                    $auth_ids = explode(',', $auth_ids);
                    foreach ($auth_ids as $key => $auth_id_v) {
                    $checked[] = Db::name('SystemAuthNode')->where('auth', $auth_id_v)->column('node');
                    }
                    $checked = call_user_func_array('array_merge', $checked); //已拥有的全部权限
                    foreach ($nodes as $key => &$node) {
                        $node['checked'] = in_array($node['node'], $checked);
                        if($node['checked']!=1){    //删除未被授权的权限
                            unset($nodes[$key]);
                        }else{
                            $node['checked'] = 0;
                        }
                    ...
                    $checked = Db::name('SystemAuthNode')->where('auth', $auth_id)->column('node');  //账户被选中的权限
                    foreach ($nodes as $key => &$node) {
                        $node['checked'] = in_array($node['node'], $checked);
                        if (empty($node['is_auth']) && substr_count($node['node'], '/') > 1) {
                            unset($nodes[$key]);
