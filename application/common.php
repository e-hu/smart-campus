...
function refundMSC($openid)
{
    ...
    ),Request::instance()->domain().'/index/index/checkList.html?company_id='.$this->getClientCompanyId());
}
...
function returnMSC($openid)
{
    sendMSC($openid,'OPENTM407446439',array(
        ...
        'keyword2'=>array('value'=>date('Y-m-d H:m:i')),
        'remark'=>array('value'=>'点击本条信息进行操作。')
    ),Request::instance()->domain().'/index/index/refund.html?company_id='.$this->getClientCompanyId());
}
...
function patchMSC($openid)
{
    sendMSC($openid,'OPENTM407446439',array(
        ...
        'keyword2'=>array('value'=>date('Y-m-d H:m:i')),
        'remark'=>array('value'=>'点击本条信息进行操作。')
    ),Request::instance()->domain().'/index/index/patch.html?company_id='.$this->getClientCompanyId());
}
