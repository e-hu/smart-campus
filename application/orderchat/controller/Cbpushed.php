...
class Cbpushed extends BasicAdmin
{
    ...
    public function index()
    {
        ...
        $tags = Db::name('dinner_base_info')->where('company_id', session('user.company_id'))->column('dinner_flag,dinner_name');
        ...
        if ($this->request->get() && isset($_GET['dinner_flag']) && isset($_GET['week_id'])) {
            ...
            $return = Db::query($sqlstr, [$weekinfo['year'], $weekinfo['week_num'], session('user.company_id'), $_GET['dinner_flag'],'2']);
            if(empty($return[0])){
                $this->error('数据不存在');
            }
            $list = $return[0];
    ...
    public function forbid()
    {
        ...
        if (Db::name('cookbook_recommend_detail')->where(['company_id' => session('user.company_id'), 'recommend_index' => $_GET['recommend_index'],'start_datetime'=> $_GET['start_datetime']])->update(['choose_flag'=>'0'])) {
    ...
    public function resume()
    {
        ...
        if (empty($_GET['recommend_index'])||empty($_GET['start_datetime']))
            $this->error("条件错误，请稍候再试！");
        if (Db::name('cookbook_recommend_detail')->where(['company_id' => session('user.company_id'), 'recommend_index' => $_GET['recommend_index'],'start_datetime'=> $_GET['start_datetime']])->update(['choose_flag'=>'1'])) {
